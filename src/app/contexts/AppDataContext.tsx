...
const INITIAL_AUTO_RULES: AutoRule[] = [
  {
    ...
    enabled: true,
    lastRun: "2026-02-12",
    conditions: [{ field: "merchant", operator: "equals", value: "Grab" }],
    actions: [{ type: "set_category", value: "cat-6" }],
  },
  {
    id: "rule-2",
    name: "Cà phê → Ăn uống",
    ...
    actions: [{ type: "set_category", value: "cat-5" }],
  },
  {
    id: "rule-3",
    name: "Lương → Thu nhập",
  ...
  },
  {
    ...
    actions: [{ type: "set_category", value: "cat-7" }],
  },
  {
    id: "rule-5",
    ...
    actions: [{ type: "add_tag", value: "tag-1" }],
  },
];
  ...
  },
  {
    ...
    executionHistory: [
      ...
      },
      {
        ...
        status: "skipped",
        note: "Bỏ qua lần chạy 2025-12-01",
      },
    ],
  },
];
...
export function AppDataProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (stored) {
        ...
        if (
          ...
          parsed.accounts[0].openingBalance === undefined
        ) {
            ...
            for (const t of parsed.transactions || []) {
              if (t.type === "income" && t.accountId === account.id) {
                ...
                if (t.accountId === account.id)
                  txnEffect -= Math.abs(t.amount) + (t.serviceFee || 0);
        ...
        if (parsed.budgets && parsed.budgets.length > 0) {
          ...
          parsed.budgets = parsed.budgets.map((b: any) => {
            if (
              b.startDate === "2026-02-01" &&
              (b.endDate === "2026-02-28" || b.endDate === "2026-02-29")
            ) {
              migrated = true;
              return {
                ...b,
                startDate: "2026-03-01",
                endDate: "2026-03-31",
                name: b.name.replace("tháng 2", "tháng 3"),
              };
            }
            return b;
          });
          ...
          if (migrated && parsed.transactions) {
            parsed.transactions = parsed.transactions.map((t: any) => {
              if (t.date && t.date.startsWith("2026-02-")) {
                ...
                return {
                  ...t,
                  date: `2026-03-${String(newDay).padStart(2, "0")}`,
                  description: t.description.replace("tháng 2", "tháng 3"),
                };
              }
              return t;
            });
  ...
  const addAccount = (
    account: Omit<Account, "id" | "lastUpdated">,
  ): Account => {
  ...
  const addBudget = (
    budget: Omit<Budget, "id" | "spent" | "status">,
  ): Budget => {
  ...
  const addGoal = (
    goal: Omit<Goal, "id" | "currentAmount" | "status">,
  ): Goal => {
    const newGoal: Goal = {
      ...goal,
      id: generateId("goal"),
  ...
  const withdrawFromGoal = (goalId: string, amount: number, notes?: string) => {
    setData((prev) => {
      ...
      const updatedGoal = {
        ...
        status:
          goal.currentAmount - amount < 0 ? ("behind" as const) : goal.status,
      };
  ...
  const reorderAutoRules = (orderedIds: string[]) => {
    setData((prev) => ({
      ...prev,
      autoRules: orderedIds.map(
        (id) => prev.autoRules.find((r) => r.id === id) as AutoRule,
      ),
    }));
  };
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "finance-hide-account-numbers",
        String(hideAccountNumbers),
      );
    } catch {}
  }, [hideAccountNumbers]);
  ...
  const restoreFullData = (
    ...
    setData((prev) => {
      ...
      const existingIds = (arr: { id: string }[]) =>
