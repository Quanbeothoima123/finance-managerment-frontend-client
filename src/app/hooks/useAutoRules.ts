...
import { useDemoData, type AutoRule, type Transaction } from '../contexts/DemoDataContext';
...
export function useAutoRules() {
  const { autoRules, transactions, categories, merchants, tags } = useDemoData();

  const evaluateRules = useCallback((form: {
    description: string;
    merchantId: string;
    accountId: string;
    amount: string;
    ...
    for (const rule of enabledRules) {
      ...
      for (const cond of rule.conditions) {
        ...
        switch (cond.field) {
          case 'description':
            if (cond.operator === 'contains' && desc.includes(condVal as string)) condMatch = true;
            if (cond.operator === 'equals' && desc === condVal) condMatch = true;
            break;
          case 'merchant':
            if (cond.operator === 'contains' && merchantName.includes(condVal as string)) condMatch = true;
            if (cond.operator === 'equals' && merchantName === (condVal as string)) condMatch = true;
            break;
          case 'amount':
            if (cond.operator === 'greater_than' && amount > (cond.value as number)) condMatch = true;
            if (cond.operator === 'less_than' && amount < (cond.value as number)) condMatch = true;
            if (cond.operator === 'equals' && amount === (cond.value as number)) condMatch = true;
            break;
        }

        if (!condMatch) { allMatch = false; break; }
   ...
   */
  const getSuggestions = useCallback((form: {
    description: string;
    merchantId: string;
    type: string;
  }): HistorySuggestion[] => {
    ...
    similar.forEach(tx => {
      if (tx.categoryId) catCounts[tx.categoryId] = (catCounts[tx.categoryId] || 0) + 1;
    });
    for (const [catId, count] of Object.entries(catCounts)) {
      const confidence = count / similar.length;
      if (confidence >= 0.5) { // Lowered from 0.7 for demo purposes
