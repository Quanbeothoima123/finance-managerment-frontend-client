...
export const recurringService = {
  ...
  listRecurringRules(query: RecurringListQuery = {}) {
    return apiRequest<RecurringListResponse>(
      `/recurring${buildQueryString(query)}`,
      {
        method: "GET",
        requiresAuth: true,
      },
  ...
  getOccurrences(ruleId: string, limit = 20) {
    return apiRequest<RecurringOccurrencesResponse>(
      `/recurring/${ruleId}/occurrences?limit=${limit}`,
      {
        method: "GET",
        requiresAuth: true,
      },
  ...
  skipNext(ruleId: string) {
    return apiRequest<RecurringDetailResponse>(
      `/recurring/${ruleId}/skip-next`,
      {
        method: "POST",
        requiresAuth: true,
      },
