...
import {
  Mail, Lock, Eye, EyeOff, Loader2, ArrowLeft, Shield,
  Wallet, CheckCircle, KeyRound, ShieldCheck, RotateCcw
} from 'lucide-react';
...
export default function AuthForgotPassword() {
  ...
  const [passwordErrors, setPasswordErrors] = useState<{ newPassword?: string; confirm?: string }>({});
  ...
  const getPasswordStrength = (pw: string): { level: 'weak' | 'medium' | 'strong'; label: string; color: string; bg: string; width: string } => {
    let score = 0;
    ...
    if (score <= 2) return { level: 'weak', label: 'Yếu', color: 'text-[var(--danger)]', bg: 'bg-[var(--danger)]', width: 'w-1/3' };
    if (score <= 3) return { level: 'medium', label: 'Trung bình', color: 'text-[var(--warning)]', bg: 'bg-[var(--warning)]', width: 'w-2/3' };
    return { level: 'strong', label: 'Mạnh', color: 'text-[var(--success)]', bg: 'bg-[var(--success)]', width: 'w-full' };
  };
  ...
  const handleResetPassword = async () => {
    ...
    else if (!/^(?=.*[A-Za-z])(?=.*\d).+$/.test(newPassword)) errs.newPassword = 'Mật khẩu phải gồm ít nhất chữ và số.';
    if (!confirmPassword) errs.confirm = 'Vui lòng nhập lại mật khẩu.';
    else if (confirmPassword !== newPassword) errs.confirm = 'Mật khẩu nhập lại không khớp.';
    setPasswordErrors(errs);
    if (Object.keys(errs).length > 0) return;

    if (!resetToken) {
      toast.error('Phiên đặt lại mật khẩu đã hết hạn. Vui lòng yêu cầu OTP mới.');
      setStep('email');
      return;
    }
        ...
        <div className="relative z-10 max-w-md">
          ...
          >
            ...
            <p className="text-white/70 mb-10">
              Chúng tôi sẽ giúp bạn khôi phục quyền truy cập an toàn. Dữ liệu tài chính luôn được bảo vệ.
            </p>

            {/* Steps visual */}
            <div className="space-y-4">
              {[
                { num: 1, label: 'Xác nhận email', desc: 'Nhập email đã đăng ký' },
                { num: 2, label: 'Xác minh OTP', desc: 'Nhập mã 6 số từ email' },
                { num: 3, label: 'Mật khẩu mới', desc: 'Tạo mật khẩu mới an toàn' },
              ].map((s, i) => {
                  ...
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-all ${
                      ...
                      {isCompleted ? (
                        ...
                        <span className={`text-sm font-semibold ${isCurrent ? 'text-white' : 'text-white/50'}`}>{s.num}</span>
                      )}
                    </div>
                    <div>
                      <p className={`text-sm font-medium ${isCurrent || isCompleted ? 'text-white' : 'text-white/50'}`}>{s.label}</p>
                      <p className={`text-xs ${isCurrent || isCompleted ? 'text-white/70' : 'text-white/30'}`}>{s.desc}</p>
                    </div>
                  </motion.div>
                );
              })}
      ...
      <div className="flex-1 flex items-center justify-center p-5 sm:p-8">
        ...
        >
          ...
          >
            ...
            {step === 'email' ? 'Quay lại đăng nhập' : step === 'success' ? 'Về đăng nhập' : 'Quay lại'}
          </button>
            ...
            {[0, 1, 2, 3].map(i => (
              <div key={i} className="flex-1 h-1 rounded-full overflow-hidden bg-[var(--border)]">
                <motion.div
                  ...
                  transition={{ duration: 0.4, delay: stepIndex >= i ? i * 0.1 : 0 }}
                />
              </div>
            ))}
          </div>
          ...
          <div className="text-center mb-8">
            <div className={`w-14 h-14 rounded-[var(--radius-xl)] flex items-center justify-center mx-auto mb-4 ${
              step === 'success' ? 'bg-[var(--success-light)]' : 'bg-[var(--primary-light)]'
            }`}>
              {currentStep.icon}
            </div>
            <h1 className="text-2xl font-semibold text-[var(--text-primary)]">{currentStep.title}</h1>
            <p className="text-[var(--text-secondary)] mt-1 text-sm">{currentStep.subtitle}</p>
          </div>
              ...
              >
                ...
                <div className="flex gap-3 p-3 mb-5 bg-[var(--info-light)] rounded-[var(--radius-lg)]">
                  ...
                  <p className="text-xs text-[var(--info)]">
                    Chúng tôi sẽ gửi mã OTP 6 số tới email này. Mã có hiệu lực trong 10 phút.
                  </p>
                </div>
              ...
              >
                ...
                <div className="mb-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-12 py-3 bg-[var(--input-background)] border rounded-[var(--radius-lg)] text-[var(--text-primary)] placeholder:text-[var(--text-disabled)] focus:outline-none focus:ring-2 transition-all ${
                        passwordErrors.newPassword ? 'border-[var(--danger)] focus:ring-[var(--danger)]/30' : 'border-[var(--border)] focus:ring-[var(--focus-ring)]/30 focus:border-[var(--primary)]'
                      }`}
                    />
                    <button
                      type="button"
                  ...
                  {passwordErrors.newPassword ? (
                    <p className="mt-1.5 text-xs text-[var(--danger)]">{passwordErrors.newPassword}</p>
                  ) : (
                  ...
                  {newPassword && pwStrength && (
                    <div className="mt-2">
                      <div className="h-1.5 bg-[var(--border)] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-300 ${pwStrength.bg} ${pwStrength.width}`} />
                      </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-12 py-3 bg-[var(--input-background)] border rounded-[var(--radius-lg)] text-[var(--text-primary)] placeholder:text-[var(--text-disabled)] focus:outline-none focus:ring-2 transition-all ${
                        passwordErrors.confirm ? 'border-[var(--danger)] focus:ring-[var(--danger)]/30' : 'border-[var(--border)] focus:ring-[var(--focus-ring)]/30 focus:border-[var(--primary)]'
                      }`}
                  ...
                  {passwordErrors.confirm && (
                    <p className="mt-1.5 text-xs text-[var(--danger)]">{passwordErrors.confirm}</p>
                  )}
                  {confirmPassword && !passwordErrors.confirm && confirmPassword === newPassword && (
            ...
            {step === 'success' && (
              ...
              >
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-2">
                  Tài khoản <span className="font-medium text-[var(--text-primary)]">{email}</span> đã được cập nhật mật khẩu mới.
                </p>
