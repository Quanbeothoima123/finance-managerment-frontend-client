...
import { ArrowLeft, PieChart as PieChartIcon, BarChart3, ChevronDown } from 'lucide-react';
...
const FALLBACK_COLORS = ['#ef4444', '#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#ec4899', '#6366f1', '#14b8a6', '#f97316', '#06b6d4'];
  ...
  const categoryData = useMemo(() => {
    ...
    return Object.entries(catMap)
      .map(([key, val], idx) => ({
        ...
        color: catLookup[key]?.color || FALLBACK_COLORS[idx % FALLBACK_COLORS.length],
        transactions: val.transactions,
      }))
      .sort((a, b) => b.amount - a.amount);
  }, [transactions, categories, selectedMonth, selectedAccount, selectedTags, tagFilterMode]);

  const totalAmount = categoryData.reduce((sum, cat) => sum + cat.amount, 0);
  const totalTransactions = categoryData.reduce((sum, cat) => sum + cat.transactions, 0);
    ...
    <div className="min-h-screen bg-[var(--background)]">
      <div className="max-w-6xl mx-auto p-4 md:p-6 pb-20 md:pb-6 space-y-6">
          ...
          <Card>
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3 flex-wrap">
                <span className="text-sm font-medium text-[var(--text-primary)]">Lọc theo tag</span>
                <TagFilterDropdown
                  tags={tags}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Card>
            <p className="text-sm text-[var(--text-secondary)] mb-2">Số giao dịch</p>
            <p className="text-2xl font-bold text-[var(--text-primary)] tabular-nums">
              {totalTransactions}
            </p>
          </Card>
          ...
          <Card>
            <p className="text-sm text-[var(--text-secondary)] mb-2">Trung bình/giao dịch</p>
            <p className="text-2xl font-bold text-[var(--text-primary)] tabular-nums">
        ...
        <Card>
          {categoryData.length === 0 ? (
            <div className="h-80 flex items-center justify-center">
              <p className="text-sm text-[var(--text-tertiary)]">Không có dữ liệu cho khoảng thời gian này</p>
            </div>
          ) : chartType === 'pie' ? (
          ...
          <Card>
            ...
            <div className="space-y-0">
              {categoryData.map((category, index) => (
                <CategoryItem key={category.id} category={category} rank={index + 1} />
              ))}
            </div>
          </Card>
        )}
