...
export default function GeneralSettings() {
  ...
  const [decimals, setDecimals] = useState(() => load<number>(STORAGE_KEYS.decimals, DEFAULTS.decimals));
  const [thousandsSep, setThousandsSep] = useState(() => load<boolean>(STORAGE_KEYS.thousandsSep, DEFAULTS.thousandsSep));
  const [symbolPosition, setSymbolPosition] = useState<'before' | 'after'>(() => load(STORAGE_KEYS.symbolPosition, DEFAULTS.symbolPosition));
  const [dateFormat, setDateFormat] = useState(() => load<string>(STORAGE_KEYS.dateFormat, DEFAULTS.dateFormat));
  const [showDayOfWeek, setShowDayOfWeek] = useState(() => load<boolean>(STORAGE_KEYS.showDayOfWeek, DEFAULTS.showDayOfWeek));
  const [autoTimezone, setAutoTimezone] = useState(() => load<boolean>(STORAGE_KEYS.autoTimezone, DEFAULTS.autoTimezone));
  const [timezone, setTimezone] = useState(() => load<string>(STORAGE_KEYS.timezone, DEFAULTS.timezone));
  ...
  const previewDate = useMemo(() => {
    ...
    const dd = '06', mm = '03', yyyy = '2026';
    const dayNames = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'];
    const dayOfWeek = dayNames[d.getDay()]; // Friday = T6

    let formatted = '';
    switch (dateFormat) {
      case 'dd/mm/yyyy': formatted = `${dd}/${mm}/${yyyy}`; break;
      case 'mm/dd/yyyy': formatted = `${mm}/${dd}/${yyyy}`; break;
      case 'yyyy-mm-dd': formatted = `${yyyy}-${mm}-${dd}`; break;
      default: formatted = `${dd}/${mm}/${yyyy}`;
    }
    ...
    <div className="min-h-screen bg-[var(--background)]">
      <div className="max-w-3xl mx-auto p-4 md:p-6 pb-28 md:pb-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-[var(--text-primary)]">Cài đặt chung</h1>
            <p className="text-sm text-[var(--text-secondary)]">Currency, định dạng số/ngày, timezone</p>
          </div>
        </div>

        {/* B2: Currency */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Tiền tệ</h3>
          <select
        ...
        {/* B3: Numeric Format */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Định dạng số</h3>

          {/* Decimals */}
          <div className="mb-4">
            <label className="block text-sm text-[var(--text-secondary)] mb-2">Số thập phân</label>
            <div className="flex gap-2">
              {[0, 2].map(d => (
                <button key={d} onClick={() => setDecimals(d)}
                  ...
                  {d} decimals {d === 0 && selectedCurrency === 'VND' ? '(mặc định VND)' : d === 2 && selectedCurrency !== 'VND' ? `(mặc định ${selectedCurrency})` : ''}
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between py-3 border-t border-[var(--divider)]">
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">Dấu phân cách hàng nghìn</p>
              <p className="text-xs text-[var(--text-tertiary)]">{thousandsSep ? '1,234,567' : '1234567'}</p>
            </div>
            <button onClick={() => setThousandsSep(!thousandsSep)}
              className={`relative w-12 h-7 rounded-full transition-colors ${thousandsSep ? 'bg-[var(--primary)]' : 'bg-[var(--border)]'}`}>
              <span className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${thousandsSep ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>

          {/* Symbol position */}
          <div className="pt-3 border-t border-[var(--divider)]">
            <label className="block text-sm text-[var(--text-secondary)] mb-2">Vị trí ký hiệu tiền</label>
            <div className="flex gap-2">
              <button onClick={() => setSymbolPosition('before')}
                className={`flex-1 px-4 py-2.5 rounded-[var(--radius-lg)] text-sm font-medium border transition-colors ${
              ...
              <button onClick={() => setSymbolPosition('after')}
                className={`flex-1 px-4 py-2.5 rounded-[var(--radius-lg)] text-sm font-medium border transition-colors ${
        ...
        {/* B4: Date Format */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Ngày tháng</h3>

          <div className="space-y-2 mb-4">
            {DATE_FORMATS.map(f => (
              <label key={f.value} className="flex items-center gap-3 p-3 rounded-[var(--radius-lg)] border border-[var(--border)] cursor-pointer hover:bg-[var(--surface)] transition-colors"
                style={dateFormat === f.value ? { borderColor: 'var(--primary)', backgroundColor: 'var(--primary-light)' } : {}}>
                <input type="radio" name="dateFormat" value={f.value} checked={dateFormat === f.value}
                ...
                <div className="flex-1 flex items-center justify-between">
                  ...
                  <span className="text-xs text-[var(--text-tertiary)] tabular-nums">{f.example}</span>
                </div>
              </label>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[var(--divider)]">
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">Hiển thị tên thứ</p>
              <p className="text-xs text-[var(--text-tertiary)]">VD: T6, 06/03/2026</p>
            </div>
            <button onClick={() => setShowDayOfWeek(!showDayOfWeek)}
              className={`relative w-12 h-7 rounded-full transition-colors ${showDayOfWeek ? 'bg-[var(--primary)]' : 'bg-[var(--border)]'}`}>
              <span className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${showDayOfWeek ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
        </Card>
        ...
        {/* B5: Timezone */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Múi giờ</h3>

          {/* Auto toggle */}
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-sm font-medium text-[var(--text-primary)]">Tự động theo thiết bị</p>
              <p className="text-xs text-[var(--text-tertiary)]">{autoTimezone ? Intl.DateTimeFormat().resolvedOptions().timeZone : 'Tắt'}</p>
            </div>
            <button onClick={() => setAutoTimezone(!autoTimezone)}
              ...
              <span className={`absolute top-0.5 left-0.5 w-6 h-6 rounded-full bg-white shadow transition-transform ${autoTimezone ? 'translate-x-5' : 'translate-x-0'}`} />
            </button>
          </div>
          ...
          {!autoTimezone && (
            <div className="pt-3 border-t border-[var(--divider)]">
              <label className="block text-sm text-[var(--text-secondary)] mb-2">Chọn múi giờ</label>
        ...
        <Card className="bg-[var(--surface)] border-l-4 border-l-[var(--primary)]">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between p-3 bg-[var(--card)] rounded-[var(--radius-lg)]">
              ...
              <span className="text-sm font-semibold text-[var(--text-primary)] tabular-nums">{previewAmount}</span>
            </div>
            <div className="flex items-center justify-between p-3 bg-[var(--card)] rounded-[var(--radius-lg)]">
              <span className="text-sm text-[var(--text-secondary)]">Ngày</span>
              <span className="text-sm font-semibold text-[var(--text-primary)] tabular-nums">{previewDate}</span>
            </div>
          </div>
        </Card>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <button onClick={handleRestoreDefaults}
            ...
            <RotateCcw className="w-4 h-4" /><span>Khôi phục mặc định</span>
          </button>
        </div>
      </div>
    </div>
