...
import { Download, FileText, File, Calendar, Filter, CheckCircle } from 'lucide-react';
...
export default function ExportCenter() {
  ...
  const accountOptions = useMemo(() => {
    return [{ id: 'all', name: 'Tất cả tài khoản' }, ...accounts.map(a => ({ id: a.id, name: a.name }))];
  }, [accounts]);
  ...
  const generateCSV = () => {
    ...
    const rows = filteredTransactions.map(txn => {
      const parts = [
        escapeCSV(txn.date),
        escapeCSV(txn.type === 'income' ? 'Thu nhập' : txn.type === 'expense' ? 'Chi tiêu' : 'Chuyển khoản'),
    ...
    toast.success(`Đã xuất ${filteredTransactions.length} giao dịch ra file CSV`);
    setExported(true);
  ...
  const generateTextReport = () => {
    ...
    lines.push(`Tài khoản: ${selectedAccount === 'all' ? 'Tất cả' : accounts.find(a => a.id === selectedAccount)?.name || selectedAccount}`);
    ...
    lines.push(`  Cân đối:         ${formatCurrency(totalIncome - totalExpense)}₫`);
    ...
    filteredTransactions
      ...
      .forEach((txn, idx) => {
        ...
        lines.push(`     ${sign}${formatCurrency(Math.abs(txn.amount))}₫ | ${txn.account}${txn.category ? ` | ${txn.category}` : ''}`);
        lines.push('');
      });
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      <div className="max-w-4xl mx-auto p-4 md:p-6 pb-20 md:pb-6 space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Trung tâm xuất dữ liệu</h1>
        ...
        <Card className="border-l-4 border-l-[var(--primary)]">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-[var(--text-primary)]">Monthly Summary (1 trang)</h3>
              <p className="text-sm text-[var(--text-secondary)] mt-0.5">Ảnh/PDF đẹp để chia sẻ</p>
            </div>
            <button onClick={() => nav.goMonthlySummary()}
        ...
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Định dạng xuất</h3>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 text-left">
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">
                    Xuất danh sách giao dịch dạng bảng để phân tích trong Excel hoặc Google Sheets
                  </p>
                </div>
              </div>
            </button>
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 text-left">
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">
                    Xuất báo cáo tháng với tổng quan thu chi và danh sách giao dịch chi tiết
                  </p>
                </div>
              </div>
            </button>
        ...
        {/* Export Options */}
        <Card>
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Tuỳ chọn xuất</h3>
        ...
        <Card className="bg-[var(--surface)]">
          <h3 className="font-semibold text-[var(--text-primary)] mb-4">Xem trước xuất</h3>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Định dạng</p>
            ...
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Số giao dịch</p>
            ...
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Tổng thu</p>
            ...
            <div>
              <p className="text-xs text-[var(--text-secondary)] mb-1">Tổng chi</p>
        ...
        <div className="flex flex-col md:flex-row gap-3">
          ...
          >
            ...
            {exported ? 'Đã xuất thành công!' : `Xuất ${filteredTransactions.length} giao dịch`}
          </Button>
