...
import { Search, ChevronLeft, TrendingUp, Users, Flame, Trophy, X, Clock, Hash } from 'lucide-react';
...
type SearchTab = 'posts' | 'people' | 'topics' | 'challenges';

const SUGGESTED_SEARCHES = ['tiết kiệm mua nhà', 'ngân sách sinh viên', 'quy tắc 50/30/20', 'quỹ khẩn cấp', 'meal prep'];

export default function CommunityDiscover() {
  const navigate = useNavigate();
  const { posts, users, topics, challenges, toggleFollow, hiddenPostIds, blockedUserIds } = useSocialData();
  const [searchQuery, setSearchQuery] = useState('');
  const [searchTab, setSearchTab] = useState<SearchTab>('posts');
  const [recentSearches, setRecentSearches] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('social_recent_searches') || '[]'); } catch { return []; }
  });
  ...
  const trendingPosts = useMemo(() =>
    [...posts]
      .filter(p => p.audience === 'public' && !hiddenPostIds.includes(p.id) && !blockedUserIds.includes(p.authorId))
      .sort((a, b) => b.likes - a.likes)
      .slice(0, 3),
  [posts, hiddenPostIds, blockedUserIds]);

  const suggestedUsers = useMemo(() =>
    users.filter(u => !u.isFollowing && u.id !== 'user-me').slice(0, 4),
  [users]);

  const q = searchQuery.toLowerCase().trim();

  const searchResults = useMemo(() => {
    if (!q) return { posts: [], people: [], topics: [], challenges: [] };
    return {
      posts: posts.filter(p => p.audience === 'public' && p.content.toLowerCase().includes(q)),
      people: users.filter(u => u.id !== 'user-me' && (u.displayName.toLowerCase().includes(q) || u.username.toLowerCase().includes(q))),
      topics: topics.filter(t => t.name.toLowerCase().includes(q) || t.description.toLowerCase().includes(q)),
      challenges: challenges.filter(c => c.title.toLowerCase().includes(q) || c.description.toLowerCase().includes(q)),
    };
  }, [q, posts, users, topics, challenges]);

  const totalResults = searchResults.posts.length + searchResults.people.length + searchResults.topics.length + searchResults.challenges.length;
  ...
  const searchTabs: { key: SearchTab; label: string; count: number }[] = [
    ...
    { key: 'people', label: 'Người dùng', count: searchResults.people.length },
    { key: 'topics', label: 'Chủ đề', count: searchResults.topics.length },
    { key: 'challenges', label: 'Thử thách', count: searchResults.challenges.length },
  ];
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="sticky top-0 z-20 bg-[var(--background)]/95 backdrop-blur-sm px-4 pt-4 pb-3 border-b border-[var(--border)]">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-lg font-bold text-[var(--text-primary)]">Khám phá</h1>
          </div>
          <div className="relative">
            ...
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-[var(--border)]">
          ...
          {isSearching && (
            <div className="flex gap-1 mt-3 overflow-x-auto scrollbar-none">
              {searchTabs.map(tab => (
                ...
                >
                  {tab.label}
                  <span className={`tabular-nums ${searchTab === tab.key ? 'text-white/80' : 'text-[var(--text-tertiary)]'}`}>
                    {tab.count}
                  </span>
                </button>
              ))}
          ...
          <div className="px-4 py-4 space-y-4 pb-24">
            {totalResults === 0 ? (
              ...
              searchResults.posts.length === 0 ? (
                <p className="text-center py-8 text-sm text-[var(--text-tertiary)]">Không có bài viết phù hợp</p>
              ) : (
              ...
              searchResults.people.length === 0 ? (
                <p className="text-center py-8 text-sm text-[var(--text-tertiary)]">Không tìm thấy người dùng</p>
              ) : (
                <Card>
              ...
              searchResults.topics.length === 0 ? (
                <p className="text-center py-8 text-sm text-[var(--text-tertiary)]">Không tìm thấy chủ đề</p>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  {searchResults.topics.map(topic => (
                    ...
                    >
                      <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-2" style={{ backgroundColor: `${topic.color}20`, color: topic.color }}>
              ...
              searchResults.challenges.length === 0 ? (
                <p className="text-center py-8 text-sm text-[var(--text-tertiary)]">Không tìm thấy thử thách</p>
              ) : (
                <div className="space-y-3">
                  {searchResults.challenges.map(ch => (
                    <Card key={ch.id}>
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl flex items-center justify-center" style={{ backgroundColor: `${ch.color}15`, color: ch.color }}>
                          <Trophy className="w-6 h-6" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-semibold text-sm text-[var(--text-primary)]">{ch.title}</p>
                          <p className="text-xs text-[var(--text-secondary)] mt-0.5">{ch.description}</p>
                        </div>
                      </div>
                    </Card>
                  ))}
            ...
            {recentSearches.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={clearRecentSearches} className="text-xs text-[var(--primary)] font-medium">
                    Xóa tất cả
                  </button>
                </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 gap-3">
                {topics.slice(0, 6).map(topic => (
                  ...
                  >
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center mb-2" style={{ backgroundColor: `${topic.color}20`, color: topic.color }}>
                      {topicIcons[topic.icon] || <TrendingUp className="w-5 h-5" />}
                    </div>
                    <p className="font-semibold text-sm text-[var(--text-primary)] mb-0.5">#{topic.name}</p>
                    <p className="text-xs text-[var(--text-tertiary)]">{topic.postsCount.toLocaleString()} bài viết</p>
                  </button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-xs text-[var(--text-tertiary)]">{ch.duration}</span>
                          <span className="text-xs text-[var(--text-tertiary)]">{ch.participantsCount.toLocaleString()} người tham gia</span>
                        </div>
                      </div>
                    </div>
                  </Card>
