...
import {
  Mail, Lock, Eye, EyeOff, Loader2, User, Calendar, CheckCircle,
  Wallet, TrendingUp, PiggyBank, BarChart3, Sparkles, Shield, RotateCcw, ArrowLeft
} from 'lucide-react';
...
const strengthConfig: Record<PasswordStrength, { label: string; color: string; bg: string; width: string }> = {
  weak: { label: 'Yếu', color: 'text-[var(--danger)]', bg: 'bg-[var(--danger)]', width: 'w-1/3' },
  medium: { label: 'Trung bình', color: 'text-[var(--warning)]', bg: 'bg-[var(--warning)]', width: 'w-2/3' },
  strong: { label: 'Mạnh', color: 'text-[var(--success)]', bg: 'bg-[var(--success)]', width: 'w-full' },
};
...
export default function AuthRegister() {
  ...
  const handleRegister = async () => {
    ...
    if (!displayName.trim()) newErrors.displayName = 'Vui lòng nhập tên hiển thị.';
    ...
    else if (!/^(?=.*[A-Za-z])(?=.*\d).+$/.test(password)) newErrors.password = 'Mật khẩu phải gồm ít nhất chữ và số.';
    if (!confirmPassword) newErrors.confirmPassword = 'Vui lòng nhập lại mật khẩu.';
    else if (confirmPassword !== password) newErrors.confirmPassword = 'Mật khẩu nhập lại không khớp.';
    if (!agreedTerms) newErrors.agreedTerms = 'Bạn phải đồng ý điều khoản sử dụng.';
    ...
    try {
      ...
      if (message.toLowerCase().includes('chưa xác minh')) {
        try {
          ...
          setStep('otp');
          setCountdown(60);
          toast.info('Email này đã đăng ký nhưng chưa xác minh. Mã OTP mới đã được gửi.');
          return;
        } catch {
        ...
        <div className="relative z-10 max-w-md">
          ...
          >
            ...
            <p className="text-white/70 mb-10">
              Chỉ mất 1 phút để bắt đầu. Theo dõi thu chi, lập ngân sách và đạt mục tiêu tiết kiệm.
            </p>

            {/* Steps */}
            <div className="space-y-4">
              {[
                { num: '1', label: 'Tạo tài khoản', desc: 'Email hoặc đăng nhập qua Google' },
                { num: '2', label: 'Thiết lập nhanh', desc: 'Chọn tiền tệ, tạo ví đầu tiên' },
                { num: '3', label: 'Bắt đầu ghi chép', desc: 'Thêm giao dịch đầu tiên' },
              ].map((s, i) => (
                <motion.div
                  key={s.num}
          ...
          {step === 'form' ? (
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Tạo tài khoản</h1>
                <p className="text-[var(--text-secondary)] mt-1">Chỉ mất 1 phút để bắt đầu</p>
              </div>
              ...
              <div className="bg-[var(--card)] rounded-[var(--radius-2xl)] border border-[var(--border)] shadow-[var(--shadow-lg)] p-6">
                {/* Display Name */}
                <div className="mb-4">
                  <label className="block text-sm font-medium text-[var(--text-primary)] mb-1.5">Tên hiển thị</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full pl-11 pr-4 py-3 bg-[var(--input-background)] border rounded-[var(--radius-lg)] text-[var(--text-primary)] placeholder:text-[var(--text-disabled)] focus:outline-none focus:ring-2 transition-all ${
                        errors.displayName ? 'border-[var(--danger)] focus:ring-[var(--danger)]/30' : 'border-[var(--border)] focus:ring-[var(--focus-ring)]/30 focus:border-[var(--primary)]'
                      }`}
                    />
                  </div>
                  {errors.displayName ? (
                    <p className="mt-1.5 text-xs text-[var(--danger)]">{errors.displayName}</p>
                  ) : (
                    <p className="mt-1.5 text-xs text-[var(--text-tertiary)]">Tên này sẽ hiển thị trong hồ sơ cá nhân.</p>
                  )}
                </div>

                {/* Date of Birth */}
                <div className="mb-4">
                  <label className="block text-sm font-medium text-[var(--text-primary)] mb-1.5">Ngày tháng năm sinh</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      max="2012-01-01"
                      className={`w-full pl-11 pr-4 py-3 bg-[var(--input-background)] border rounded-[var(--radius-lg)] text-[var(--text-primary)] placeholder:text-[var(--text-disabled)] focus:outline-none focus:ring-2 transition-all ${
                        errors.dob ? 'border-[var(--danger)] focus:ring-[var(--danger)]/30' : 'border-[var(--border)] focus:ring-[var(--focus-ring)]/30 focus:border-[var(--primary)]'
                      }`}
                    />
                  </div>
                  {errors.dob ? (
                    <p className="mt-1.5 text-xs text-[var(--danger)]">{errors.dob}</p>
                  ) : (
                    <p className="mt-1.5 text-xs text-[var(--text-tertiary)]">Dùng để cá nhân hóa trải nghiệm.</p>
                  )}
                </div>
                ...
                <div className="mb-4">
                  ...
                  {errors.password ? (
                    <p className="mt-1.5 text-xs text-[var(--danger)]">{errors.password}</p>
                  ) : (
                    <p className="mt-1.5 text-xs text-[var(--text-tertiary)]">Tối thiểu 8 ký tự, gồm chữ và số.</p>
                  )}
                  ...
                  {password && passwordStrength && (
                    <div className="mt-2">
                      <div className="h-1.5 bg-[var(--border)] rounded-full overflow-hidden">
                        <div className={`h-full rounded-full transition-all duration-300 ${strengthConfig[passwordStrength].bg} ${strengthConfig[passwordStrength].width}`} />
                      </div>
                      <p className={`text-xs mt-1 ${strengthConfig[passwordStrength].color}`}>
                ...
                {/* Confirm Password */}
                <div className="mb-5">
                  <label className="block text-sm font-medium text-[var(--text-primary)] mb-1.5">Nhập lại mật khẩu</label>
                  <div className="relative">
                    <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-[var(--text-tertiary)]" />
                    <input
                      ...
                      className={`w-full pl-11 pr-12 py-3 bg-[var(--input-background)] border rounded-[var(--radius-lg)] text-[var(--text-primary)] placeholder:text-[var(--text-disabled)] focus:outline-none focus:ring-2 transition-all ${
                        errors.confirmPassword ? 'border-[var(--danger)] focus:ring-[var(--danger)]/30' : 'border-[var(--border)] focus:ring-[var(--focus-ring)]/30 focus:border-[var(--primary)]'
                      }`}
                    />
                    <button
                      type="button"
                  ...
                  {errors.confirmPassword && <p className="mt-1.5 text-xs text-[var(--danger)]">{errors.confirmPassword}</p>}
                  {confirmPassword && !errors.confirmPassword && confirmPassword === password && (
                    <div className="flex items-center gap-1 mt-1.5">
                      <CheckCircle className="w-3.5 h-3.5 text-[var(--success)]" />
                      <p className="text-xs text-[var(--success)]">Mật khẩu khớp</p>
                    </div>
                  )}
                </div>
                ...
                <label className="flex items-start gap-3 mb-6 cursor-pointer">
                  ...
                  <span className="text-sm text-[var(--text-secondary)]">
                    Tôi đồng ý với{' '}
                    <button onClick={() => toast.info('Trang Điều khoản sử dụng.')} className="text-[var(--primary)] hover:underline font-medium">
                      Điều khoản sử dụng
                    </button>
                    {' '}và{' '}
                    <button onClick={() => toast.info('Trang Chính sách bảo mật.')} className="text-[var(--primary)] hover:underline font-medium">
                      Chính sách bảo mật
                    </button>
                  </span>
                </label>
              ...
              <div className="mt-6">
                <div className="flex items-center gap-4 mb-5">
                  ...
                  <span className="text-xs text-[var(--text-tertiary)]">Hoặc đăng ký với</span>
                  <div className="flex-1 h-px bg-[var(--border)]" />
                </div>
              ...
              <p className="text-center text-sm text-[var(--text-secondary)] mt-8 pb-4">
                Đã có tài khoản?{' '}
                <Link to="/auth/login" className="font-medium text-[var(--primary)] hover:underline">
                  Đăng nhập
                </Link>
              </p>
            </motion.div>
            ...
            >
              ...
              <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Xác minh Email</h1>
                <p className="text-[var(--text-secondary)] mt-2 text-sm max-w-xs mx-auto">
                  Chúng tôi vừa gửi một mã OTP gồm 6 chữ số đến email của bạn. Vui lòng nhập mã để xác minh.
                </p>
              </div>
              ...
              <div className="bg-[var(--card)] rounded-[var(--radius-2xl)] border border-[var(--border)] shadow-[var(--shadow-lg)] p-6">
                ...
                <div className="flex items-center justify-center gap-2 mb-6 p-3 bg-[var(--surface)] rounded-[var(--radius-lg)]">
                  ...
                  <span className="text-sm text-[var(--text-primary)] font-medium">{email}</span>
                </div>
                ...
                {/* Demo hint */}
                <div className="flex gap-3 p-3 mb-6 bg-[var(--surface)] rounded-[var(--radius-lg)]">
              ...
              <div className="mt-6 text-center">
                <button
                  onClick={() => { setStep('form'); setOtpValues(['', '', '', '', '', '']); setOtpError(''); }}
