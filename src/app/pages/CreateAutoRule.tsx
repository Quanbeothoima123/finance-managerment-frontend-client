...
const PRIMARY_FIELDS: AutoRuleMatchField[] = ['description', 'merchant', 'note'];
...
export default function CreateAutoRule({ mode = 'create' }: CreateAutoRuleProps) {
  const nav = useAppNavigation();
  const toast = useToast();
  const { id } = useParams<{ id: string }>();
  const isEdit = mode === 'edit';

  const { data: meta, loading: metaLoading, error: metaError } = useAutoRulesMeta();
  const {
    data: detail,
  ...
  const [matchField, setMatchField] = useState<AutoRuleMatchField>('description');
  ...
  useEffect(() => {
    ...
    setMatchField(rule?.matchField || meta.defaults.matchField || 'description');
  ...
  const availableFields = useMemo(
    () => (meta?.supportedFields || []).filter((item) => PRIMARY_FIELDS.includes(item.value)),
    [meta?.supportedFields],
  );
  const availableOperators = useMemo(
    () => (meta?.supportedOperators || []).filter((item) => PRIMARY_OPERATORS.includes(item.value)),
  ...
  const payload = useMemo<CreateAutoRulePayload>(
    ...
    [active, matchField, matchType, name, pattern, priority, selectedCategory, selectedMerchant, selectedTags],
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : 'Không thể lưu quy tắc tự động');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleTest = () => {
    ...
    if (selectedCategoryLabel) actions.push(`Danh mục: ${selectedCategoryLabel}`);
    if (selectedMerchantLabel) actions.push(`Merchant: ${selectedMerchantLabel}`);
    if (selectedTagLabels.length > 0) actions.push(`Tags: ${selectedTagLabels.join(', ')}`);
  ...
  if (metaLoading || (isEdit && detailLoading) || !initialized) {
    return (
      <div className="p-4 md:p-6">
        <Card>
          <p className="text-[var(--text-secondary)]">Đang tải biểu mẫu quy tắc tự động...</p>
        </Card>
      </div>
    );
  }
      ...
      <div className="p-4 md:p-6">
        <Card>
          <p className="text-[var(--danger)]">
            {metaError || detailError || 'Không thể tải dữ liệu quy tắc tự động'}
          </p>
    ...
    <div className="min-h-screen bg-[var(--background)]">
      <div className="max-w-3xl mx-auto p-4 md:p-6 pb-20 md:pb-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  min="1"
                />
                <p className="text-xs text-[var(--text-secondary)] mt-1">Số nhỏ = ưu tiên cao hơn</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                value={matchField}
                onChange={(event) => setMatchField(event.target.value as AutoRuleMatchField)}
            ...
            <div>
              ...
              <Input
                ...
                placeholder={matchType === 'regex' ? 'VD: (grab|gojek|be)' : 'VD: grab'}
              />
              {matchType === 'regex' && (
                <p className="text-xs text-[var(--text-secondary)] mt-1">Sử dụng cú pháp JavaScript RegExp</p>
              )}
            </div>
          </div>
        </Card>
        ...
        <div className="flex flex-col-reverse md:flex-row gap-3">
          ...
          <Button onClick={() => void handleSave()} disabled={saving} className="flex-1 md:flex-initial">
