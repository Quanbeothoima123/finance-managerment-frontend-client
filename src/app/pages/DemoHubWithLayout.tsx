...
export default function DemoHubWithLayout() {
  ...
  const renderGroup = (group: RouteGroup, routes: typeof routeConfigs) => {
    ...
    return (
      <div
        key={group}
        className="bg-[var(--card)] rounded-[var(--radius-xl)] border border-[var(--border)] overflow-hidden"
      >
        {/* Group Header */}
        <div className="px-6 py-4 bg-[var(--surface)] border-b border-[var(--border)] flex items-center gap-3">
          <div
            className={`p-2 rounded-[var(--radius-md)] bg-[var(--surface-elevated)] ${meta.color}`}
          >
      ...
      <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[var(--text-secondary)] max-w-2xl">
                Quick access to all screens for QA testing. Click any button
                below to navigate to that screen. All routes are organized by
                feature group.
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            <div className="bg-[var(--card)] rounded-[var(--radius-lg)] p-4 border border-[var(--border)]">
              <div className="text-2xl font-bold text-[var(--text-primary)]">
                {groupedRoutes.core.length +
                  groupedRoutes.accounts.length +
                  groupedRoutes.categories.length}
              </div>
              <div className="text-xs text-[var(--text-tertiary)]">
                Core Screens
              </div>
          ...
          <div className="bg-[var(--card)] rounded-[var(--radius-xl)] border border-[var(--border)] overflow-hidden">
            <div className="px-6 py-4 bg-[var(--surface)] border-b border-[var(--border)] flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-tertiary)]">
                  Preview special UI states without modifying data
                </p>
              </div>
            </div>
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-[var(--info-light)] border border-[var(--info)] rounded-[var(--radius-lg)] p-6">
          ...
          <ul className="text-sm text-[var(--text-secondary)] space-y-1">
            ...
            <li>
              • Active states are automatically highlighted in sidebar/bottom
              nav
            </li>
            <li>• Test responsive behavior by resizing your browser</li>
            <li>
              • Mobile bottom nav "Thêm" (More) button opens a drawer with all
              routes
            </li>
            <li>• Theme toggle works globally and persists in localStorage</li>
            <li>
              • All cards and lists have click handlers that navigate to detail
              pages
            </li>
          </ul>
        </div>
