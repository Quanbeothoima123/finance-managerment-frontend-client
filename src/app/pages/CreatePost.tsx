...
import { ChevronDown, Image, BarChart3, Target, HelpCircle, FileText, Globe, Users, Lock, Eye, X } from 'lucide-react';
import { useSocialData, type PostType, type Audience, type FinanceRecapData } from '../contexts/SocialDataContext';
...
const POST_TYPES: { type: PostType; label: string; icon: React.ReactNode }[] = [
  { type: 'article', label: 'Bài viết', icon: <FileText className="w-4 h-4" /> },
  ...
  { type: 'question', label: 'Câu hỏi', icon: <HelpCircle className="w-4 h-4" /> },
];

const TOPIC_OPTIONS = ['Tiết kiệm', 'Budget', 'Mục tiêu', 'Chi tiêu gia đình', 'Sinh viên', 'Thu nhập phụ', 'Đầu tư cơ bản'];

const audienceConfig: Record<Audience, { label: string; icon: React.ReactNode }> = {
  public: { label: 'Công khai', icon: <Globe className="w-3.5 h-3.5" /> },
  followers: { label: 'Người theo dõi', icon: <Users className="w-3.5 h-3.5" /> },
...
export default function CreatePost() {
  ...
  const [attachedRecap, setAttachedRecap] = useState<FinanceRecapData | null>(null);
  ...
  const handlePost = () => {
    ...
    addPost({
      ...
      audience,
      ...(attachedRecap ? { recapData: attachedRecap } : {}),
    });
    toast.success(isDraft ? 'Đã lưu bản nháp!' : 'Bài viết đã được đăng thành công!');
    navigate('/community');
  };
      ...
      <div className="min-h-screen bg-[var(--background)]">
        <div className="max-w-2xl mx-auto">
          <div className="sticky top-0 z-20 bg-[var(--background)]/95 backdrop-blur-sm border-b border-[var(--border)] px-4 py-3 flex items-center justify-between">
            <button onClick={() => setShowPreview(false)} className="text-sm font-medium text-[var(--text-secondary)]">
              ← Chỉnh sửa
            </button>
            <h1 className="font-semibold text-[var(--text-primary)]">Xem trước</h1>
            <button onClick={handlePost} disabled={!canPost} className="px-4 py-1.5 rounded-full text-sm font-semibold bg-[var(--primary)] text-white hover:bg-[var(--primary-hover)]">
          ...
          <div className="px-4 py-6">
            <div className="bg-[var(--card)] rounded-[20px] border border-[var(--border)] overflow-hidden">
              <div className="px-4 pt-4 pb-2 flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm text-[var(--text-primary)]">{currentUser.displayName}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-[var(--text-tertiary)]">Vừa xong</span>
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium ${
                      audience === 'public' ? 'bg-[var(--success-light)] text-[var(--success)]' :
                      audience === 'followers' ? 'bg-[var(--primary-light)] text-[var(--primary)]' :
              ...
              {selectedTopics.length > 0 && (
                <div className="px-4 pb-3 flex flex-wrap gap-1.5">
                  {selectedTopics.map(t => <TopicChip key={t} label={t} size="sm" />)}
                </div>
              )}
            </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="sticky top-0 z-20 bg-[var(--background)]/95 backdrop-blur-sm border-b border-[var(--border)] px-4 py-3 flex items-center justify-between">
          <button onClick={() => navigate(-1)} className="text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
            Hủy
          </button>
          <h1 className="font-semibold text-[var(--text-primary)]">Tạo bài viết</h1>
          <div className="flex items-center gap-2">
            {canPost && (
              <button onClick={() => setShowPreview(true)} className="p-2 rounded-xl hover:bg-[var(--surface)] text-[var(--text-secondary)]">
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-semibold transition-colors ${
                canPost ? 'bg-[var(--primary)] text-white hover:bg-[var(--primary-hover)]' : 'bg-[var(--surface)] text-[var(--text-tertiary)]'
              }`}
            >
              {isDraft ? 'Lưu nháp' : 'Đăng'}
            </button>
        ...
        <div className="px-4 py-4 space-y-5">
          ...
          <div className="flex items-center gap-3">
            <img src={currentUser.avatar} alt={currentUser.displayName} className="w-10 h-10 rounded-full object-cover" />
            <div>
              <p className="font-semibold text-sm text-[var(--text-primary)]">{currentUser.displayName}</p>
              <button
          ...
          {attachedRecap && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-[var(--text-primary)]">Recap đính kèm</p>
                <button onClick={() => navigate('/community/share-recap')} className="text-xs text-[var(--primary)] font-medium">
                  Tùy chỉnh
                </button>
              </div>
              <FinanceRecapCard
                data={attachedRecap}
                showPrivacyHint
                onRemove={() => { setAttachedRecap(null); toast.info('Đã gỡ recap đính kèm'); }}
              />
            </div>
          )}

          {/* Attach Recap Button */}
          {!attachedRecap && (postType === 'recap' || postType === 'milestone') && (
            <div className="flex gap-2">
              ...
              >
                ...
                <p className="text-sm font-medium text-[var(--text-secondary)]">
                  {postType === 'recap' ? 'Đính kèm Recap' : 'Đính kèm tiến độ'}
                </p>
                <p className="text-xs text-[var(--text-tertiary)] mt-0.5">Tạo card từ dữ liệu của bạn</p>
              </button>
              <button
              ...
              >
                ...
                <p className="text-sm font-medium text-[var(--text-secondary)]">Tùy chỉnh Recap</p>
                <p className="text-xs text-[var(--text-tertiary)] mt-0.5">Chọn dữ liệu hiển thị</p>
              </button>
            </div>
          )}
          ...
          {/* Topic Selector */}
          <div>
            <p className="text-sm font-medium text-[var(--text-primary)] mb-2">Chủ đề</p>
        ...
        {showAudienceSheet && (
          <>
            <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setShowAudienceSheet(false)} />
            <div className="fixed bottom-0 left-0 right-0 bg-[var(--card)] rounded-t-3xl z-50 safe-area-inset-bottom">
              <div className="flex justify-center pt-3 pb-1">
                <div className="w-10 h-1 rounded-full bg-[var(--border)]" />
              </div>
              <div className="p-6 pt-4">
                <h3 className="font-semibold text-[var(--text-primary)] mb-4">Ai có thể thấy bài viết này?</h3>
                <AudienceSelector value={audience} onChange={(v) => { setAudience(v); setShowAudienceSheet(false); }} />
                <button onClick={() => setShowAudienceSheet(false)} className="w-full mt-4 py-3 text-sm font-medium text-[var(--text-secondary)] bg-[var(--surface)] rounded-2xl">
                  Đóng
                </button>
              </div>
            </div>
